<script>
	...
	function createSeededDarkColorGenerator(seed, maxBrightness = 128) {
	...
	function randomChar(kuda) {
		...
		const hex = randomBrightColor(Math.floor(random() * 1000));

		var hairHex = hex;
		if (randomSame == 1) {
			hairHex = createSeededDarkColorGenerator(
...
<main class="container mx-auto px-6 py-16 relative">
	...
	>
		...
		>
			<button
				class="absolute right-4 md:right-10 hover:scale-125 transition duration-300"
				onclick={closeCard}
			>
				<svg
			...
			{#if selectKuda}
				<div class="w-full max-w-2xl mx-auto">
					...
					<div class="mt-2 flex items-center space-x-3 text-gray-600">
						<span class="text-md md:text-lg">Owner: {selectKuda.owner}</span>
					</div>
					...
					<div class="my-4 md:my-10">
						<div class="mt-2 md:mt-6 flex items-center w-full">
							...
							></div>
							<span class="mx-4 text-gray-300 text-2xl font-light"
								>
									<img src="horseshoe_1.svg" alt="horseshoe_1.svg" class="w-5 h-5">
								</span
							>
							<div
								class="flex-grow border-t border-gray-300"
					...
					<!-- Character Details -->
					<div class="space-y-3 text-sm grid grid-cols-2 md:grid-cols-1">
